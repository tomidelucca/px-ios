...
class TermsAndConditionsViewCell: UITableViewCell, UITextViewDelegate {
    ...
    private static func getTyCText() -> NSMutableAttributedString{
        ...
        let normalAttributes: [String:AnyObject] = [NSFontAttributeName : UIFont(name:MercadoPagoContext.getDecorationPreference().getFontName(), size: 12) ?? UIFont.systemFont(ofSize: 12),NSForegroundColorAttributeName: UIColor.px_grayLight()]
