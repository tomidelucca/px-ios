...
open class CardFormViewController: MercadoPagoUIViewController , UITextFieldDelegate {
    ...
    fileprivate func prepareCVVLabelForEdit(){
        ...
        if(!self.cardFormManager.isAmexCard(self.cardNumberLabel!.text!)){
            UIView.transition(from: self.cardFront!, to: self.cardBack!, duration: 0.6, options: UIViewAnimationOptions.transitionFlipFromLeft, completion: { (completion) -> Void in
    ...
    func leftArrowKeyTapped(){
        ...
        case cvvLabel! :
            if (self.cardFormManager.getGuessedPM()?.secCodeInBack())!{
                UIView.transition(from: self.cardBack!, to: self.cardFront!, duration: 0.6, options: UIViewAnimationOptions.transitionFlipFromRight, completion: { (completion) -> Void in
                })
            }
    ...
    func makeToken(){
        ...
        if cardFormManager.customerCard != nil {
            ...
            if(cardFormManager.getGuessedPM()!.isSecurityCodeRequired(bin)){
                ...
                if((errorCVV) != nil){
                    markErrorLabel(cvvLabel!)
                    UIView.transition(from: self.cardBack!, to: self.cardFront!, duration: 0.6, options: UIViewAnimationOptions.transitionFlipFromLeft, completion: nil)
                    return
                }
            }
        }
