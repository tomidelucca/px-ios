    ...
    func createNewCardToken() {
        ...
        self.viewModel.mercadoPagoServicesAdapter.createToken(cardToken: self.viewModel.cardToken!, callback: { [weak self] (token) in
            ...
            if error.apiException?.containsCause(code: ApiUtil.ErrorCauseCodes.INVALID_IDENTIFICATION_NUMBER.rawValue) == true {
                if let identificationViewController = strongSelf.pxNavigationController.navigationController.viewControllers.last as? IdentificationViewController {
                    identificationViewController.showErrorMessage("Revisa este dato".localized)
                }
            } else {
                strongSelf.pxNavigationController.dismissLoading()
...
extension OneTapFlow {
    ...
    func getPayerCosts(updateCallback: (() -> Void)? = nil) {
        ...
        self.viewModel.mercadoPagoServicesAdapter.getInstallments(bin: nil, amount: self.viewModel.getAmount(), issuer: self.viewModel.paymentData.getIssuer(), paymentMethodId: paymentMethod.paymentMethodId, callback: { (installments) in

            self.viewModel.payerCosts = installments.first?.payerCosts
            if let payerCost = installments.first?.payerCosts.first {
                self.viewModel.updateCheckoutModel(payerCost: payerCost)
                self.executeNextStep()
            } else {
                let error = MPSDKError(message: "Hubo un error".localized, errorDetail: "", retry: true)
                error.requestOrigin = ApiUtil.RequestOrigin.GET_INSTALLMENTS.rawValue
                self.pxNavigationController.showErrorScreen(error: error, callbackCancel: self.exitCallback, errorCallback: {
                    self.getPayerCosts()
                })
            }
        }, failure: {(error) in

            let error = MPSDKError.convertFrom(error, requestOrigin: ApiUtil.RequestOrigin.GET_INSTALLMENTS.rawValue)
            error.retry = false
            self.pxNavigationController.showErrorScreen(error: error, callbackCancel: self.exitCallback, errorCallback: nil)
        })
    }
}
