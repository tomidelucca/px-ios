...
open class MPStepBuilder : NSObject {
    ...
    open class func startCreditCardForm(_ paymentSettings : PaymentPreference? = nil , amount: Double, cardInformation: CardInformation? = nil, paymentMethods : [PaymentMethod]? = nil, token: Token? = nil, timer : CountdownTimer? = nil, callback : @escaping ((_ paymentMethod: PaymentMethod, _ token: Token? ,  _ issuer: Issuer?) -> Void), callbackCancel : ((Void) -> Void)?) -> MPNavigationController {
        ...
        func verifyPaymentMethods(paymentMethods: [PaymentMethod]) -> Bool{
            if paymentMethods.count == 2 {
                if paymentMethods[0].paymentTypeId == "credit_card" || paymentMethods[1].paymentTypeId == "credit_card" {
                    if paymentMethods[0].paymentTypeId == "debit_card" || paymentMethods[1].paymentTypeId == "debit_card"{
                        return true
                    }
                }
            }
        ...
        ccf = CardFormViewController(paymentSettings : paymentSettings , amount: amount, token: token, cardInformation: cardInformation, paymentMethods : paymentMethods, timer : timer, callback : { (paymentMethod, cardToken) -> Void in
            ...
            if(paymentMethod[0].isIdentificationRequired()){
                let identificationForm = MPStepBuilder.startIdentificationForm({ (identification) -> Void in
                    ...
                    if verifyPaymentMethods(paymentMethods: paymentMethod){
                        let creditDebitForm = startCreditDebitForm(paymentMethod, issuer: nil, token: cardToken, amount: amount, timer: timer, callback: { (selectedPaymentMethod) in
                        ...
                        self.getIssuers(paymentMethod[0], cardToken: cardToken!, customerCard: cardInformation, timer: timer, ccf: ccf, callback: callback)
                    }
                    }, timer : timer)
    ...
    open class func startCreditDebitForm(_ paymentMethod : [PaymentMethod] , issuer:Issuer?, token : CardToken? , amount: Double, paymentPreference: PaymentPreference? = nil, timer: CountdownTimer? = nil,
        ...
        return PayerCostStepViewController(paymentMethod: paymentMethod, issuer: issuer, token: token, amount: amount, paymentPreference: paymentPreference, installment : nil, timer: timer, callback: callback )
    }
    ...
    open class func startPayerCostForm(_ paymentMethod : [PaymentMethod] , issuer:Issuer?, token : Token? , amount: Double, paymentPreference: PaymentPreference? = nil, installment : Installment? = nil, timer: CountdownTimer? = nil,
        ...
        MercadoPagoContext.initFlavor2()
        return PayerCostStepViewController(paymentMethod: paymentMethod, issuer: issuer, token: token, amount: amount, paymentPreference: paymentPreference, installment: installment, timer: timer, callback: callback)
    }
    ...
    open class func startIssuerForm(_ paymentMethod: PaymentMethod, cardToken: CardToken, issuerList: [Issuer]? = nil, timer : CountdownTimer? = nil,
                                    callback : @escaping ((_ issuer: NSObject?) -> Void)) -> PayerCostStepViewController {
        
        MercadoPagoContext.initFlavor2()
        return PayerCostStepViewController(paymentMethod: [paymentMethod], issuer: nil, token: cardToken, amount: nil, paymentPreference: nil, installment : nil, timer: timer, callback: callback)
