...
public class PaymentVaultViewController: MercadoPagoUIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    internal init(amount: Double, currencyId : String, purchaseTitle : String, paymentMethodSearch : [PaymentMethodSearchItem], paymentMethodSearchParent : PaymentMethodSearchItem, title: String!, callback: (paymentMethod: PaymentMethod, token: Token?, issuer: Issuer?, installments: Int) -> Void) {
            ...
            if paymentSearchItemSelected.idPaymentMethodSearchItem == "account_money" {
                ...
                MPServicesBuilder.getPaymentMethods({ (paymentMethods) -> Void in
                        if paymentMethods?.count > 0 {
                            ...
                            self.callback(paymentMethod: paymentMethodSelected!, token: nil, issuer: nil, installments: 1)
                        } else {
                            //TODO
                        }
