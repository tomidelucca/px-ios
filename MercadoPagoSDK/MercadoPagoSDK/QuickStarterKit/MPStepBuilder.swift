...
public class MPStepBuilder : NSObject {
    ...
    public class func startCreditCardForm(paymentSettings : PaymentPreference? , amount: Double, token: Token? = nil ,callback : ((paymentMethod: PaymentMethod, token: Token? ,  issuer: Issuer?, installment: Installment?) -> Void), callbackCancel : (Void -> Void)?) -> CardFormViewController {
        
        
        return CardFormViewController(paymentSettings : paymentSettings , amount: amount, token: token, callback : callback, callbackCancel: callbackCancel)
    }
