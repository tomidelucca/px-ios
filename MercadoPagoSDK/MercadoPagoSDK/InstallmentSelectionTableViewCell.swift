...
class InstallmentSelectionTableViewCell: UITableViewCell {
    ...
    func fillCell(_ payerCost : PayerCost) {
        ...
        let totalAttributes: [String:AnyObject] = [NSFontAttributeName : UIFont(name: MercadoPagoContext.getDecorationPreference().getFontName(), size: 23) ?? UIFont.systemFont(ofSize: 23),NSForegroundColorAttributeName:mpLightGrayColor]
        let noRateAttributes = [NSForegroundColorAttributeName : UIColor(red: 67, green: 176,blue: 0), NSFontAttributeName : UIFont(name:MercadoPagoContext.getDecorationPreference().getFontName(), size: 13) ?? UIFont.systemFont(ofSize: 13)]
