...
class CountdownTimerTest: BaseTest, TimerDelegate {
    ...
    func testSetup(){
        ...
        CountdownTimer.getInstance().setup(seconds: 1, timeoutCallback: {(Void) -> Void in
            exp.fulfill()
        })
    ...
    func testCurrentTiming(){
        CountdownTimer.getInstance().setup(seconds: 45, timeoutCallback: {(Void) -> Void in
            self.expectation(description: "timeout")
        })
        XCTAssertEqual("00 : 45", CountdownTimer.getInstance().getCurrentTiming())
        
        CountdownTimer.getInstance().setup(seconds: 60, timeoutCallback: {(Void) -> Void in
            self.expectation(description: "timeout")
        })
        XCTAssertEqual("01 : 00", CountdownTimer.getInstance().getCurrentTiming())
        
        CountdownTimer.getInstance().setup(seconds: 270, timeoutCallback: {(Void) -> Void in
            self.expectation(description: "timeout")
        })
        XCTAssertEqual("04 : 30", CountdownTimer.getInstance().getCurrentTiming())
        
        CountdownTimer.getInstance().setup(seconds: 600, timeoutCallback: {(Void) -> Void in
            self.expectation(description: "timeout")
        })
        XCTAssertEqual("10 : 00", CountdownTimer.getInstance().getCurrentTiming())
        
        CountdownTimer.getInstance().setup(seconds: 3599, timeoutCallback: {(Void) -> Void in
            self.expectation(description: "timeout")
        })
        XCTAssertEqual("59 : 59", CountdownTimer.getInstance().getCurrentTiming())
        
        CountdownTimer.getInstance().setup(seconds: 3600, timeoutCallback: {(Void) -> Void in
            self.expectation(description: "timeout")
        })
